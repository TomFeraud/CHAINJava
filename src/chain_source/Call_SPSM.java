...
/**
 * This class is responsible for initially taking in both the target and source
 * schema before calling SPSM with these schemas it will then store the results
 * as an ArrayList of Match_Struc objects
 * 
 * This class is tested in SPSM_Test_Cases.java and
 * SPSM_Filter_Results_Test_Cases.java
 * 
 * 
 * @author Diana Bental
 * @author Tanya Howden Date Septembre 2017 (Modified November 2017)
 *
 */
public class Call_SPSM {
	...
	public static void main(String[] args) {
		...
		String target = "waterBodyPressures(dataSource,identifiedDate,affectsGroundwater,waterBodyId);waterBodyPressures(apple,banane)";

		// source="author(name)";
		// target="document(title,author) ; author(name,document) ;
	...
	/**
	 * Call SPSM on the source schema and one or more target schemas. If the schemas
	 * are not passed as a parameter then get them from the user
	 * 
	 * @param results
	 * @param srcSchema
	 * @param targetSchemas
	...
	/**
	 * Makes call to SPSM through using .sh file. Match one source schema to one
	 * target schema. May return multiple matches
	 * 
	 * @param results
	 * @param currTarget
	 * 
	...
	public ArrayList<Match_Struc> callSPSMOnce(ArrayList<Match_Struc> results, String currTarget) {
		...
		// System.out.println("Call_SPSM.java: Reporting the match structures created by
		// SPSM") ;
	...
	public ArrayList<Match_Struc> parseMatchObject(ArrayList<Match_Struc> results, String targetSchema,
		...
		// then add this new match to our overall list of results, if there have been
		// matches only
