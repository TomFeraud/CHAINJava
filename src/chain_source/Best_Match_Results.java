...
/**
 * Responsible for taking results after calling SPSM and then filtering to
 * ensure each result is over the determined threshold, cut the number of
 * results to the desired n and sort so that the match with the highest
 * similarity is at the top of the list
 * 
 * This class is tested in Match_Results_Test_Cases.java and
...
public class Best_Match_Results {
	...
	/**
	 * Start off by taking in the results, threshold value and the number of results
	 * wanted if these values haven't been passed in through params, then ask user
	 * to enter in console
	 * 
	 * @param results
	 * @param threshVal
	...
	public ArrayList<Match_Struc> getThresholdAndFilter(ArrayList<Match_Struc> results, Double threshVal, int limNum) {
		...
		if (threshVal == null && limNum == 0) {
			...
			System.out.println(
					"If you would like to limit the number of results then please enter the limit, if not enter '0'");
	...
	/**
	 * this method will sort the resulting matches after they have been filtered to
	 * ensure that only those over or equal to the threshold value are left
	 * 
	 * @param filteredRes
	 * @param limitNo
	 * 
