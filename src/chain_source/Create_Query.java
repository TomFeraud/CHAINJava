...
/**
 * Creates a (repaired) query directly from a (repaired) schema, plus
 * information about data bindings previously extracted from the original query.
 * 
 * This query may be created without either any data bindings, or with all the
 * original data bindings
 * 
 * Responsible for using the repaired schema to create either a sepa or dbpedia
 * query, this query is added as a field onto the Match_Struc object structure
 * 
 ...
 * @author Tanya Howden
 * 
 *         Date September 2017 Modified December 2017
 *
 */
public class Create_Query {
	...
	public static void main(String[] args) {
		...
		// String
		// source="surfaceWaterBodies(subBasinDistrict,riverName,altitudeTypology,associatedGroundwaterId,nonsense)";
		// String
		// target="surfaceWaterBodies(subBasinDistrict,riverName,altitudeTypology,associatedGroundwaterId,nonsense)";
		// String qtype = "sepa" ;
		// String dataDirectory = "queryData/sepa/sepa_datafiles/";
		// String ontologyPath = "queryData/sepa/sepa_ontology.json" ;
		// int maxResults = 0;
		// String originalQuery = "PREFIX geo:
		...
		finalRes = queryCreator.createQueries(finalRes, queryData, qtype, dataDirectory, ontologyPath, WITHDATA,
				maxResults);
		// finalRes = queryCreator.createQueries(finalRes, queryData , qtype,
		// dataDirectory, ontologyPath, NODATA, maxResults);
	 ...
	 * @return
	 */
	public ArrayList<Match_Struc> createQueries(ArrayList<Match_Struc> matchRes, Query_Data queryData, String queryType,
			String datasetDir, String ontologyFilePath, int noResults) {
	 ...
	 * @return
	 */
	public ArrayList<Match_Struc> createOpenQueries(ArrayList<Match_Struc> matchRes, Query_Data queryData,
			String queryType, String datasetDir, String ontologyFilePath, int noResults) {
		return createQueries(matchRes, queryData, queryType, datasetDir, ontologyFilePath, NODATA, noResults);
	}

	/**
	 * sets up ready to create query for each repaired schema in list of match
	 * structures. Parameter sets whether to bind object variables or not
	 * 
	 * @param matchRes
	 * @param queryData
	 * @param queryType
	 ...
	 */
	public ArrayList<Match_Struc> createQueries(ArrayList<Match_Struc> matchRes, Query_Data queryData, String queryType,
			String datasetDir, String ontologyFilePath, int withBindings, int noResults) {
		String type = "";
		...
		if (ontologies != null) {
			for (int i = 0; i < matchRes.size(); i++) {
				...
				String query = createQuery(type, matchDetails, queryData, datasetDir, withBindings, noResults,
						ontologies);
	...
	/**
	 * Read the ontology file from the path as a String and make the (target)
	 * ontology structures stored in a list of Ontology_Struc
	 * 
	 * @param ontologyFilePath	 
	 * @return ontologies
	 */
	...
	public String createSepaQuery(Match_Struc matchDetails, Query_Data queryData, String datafileDir, int withBindings,
			int noResults, ArrayList<Ontology_Struc> ontologies) {
	...
	public String createDbpediaQuery(Match_Struc matchDetails, Query_Data queryData, int withBindings, int noResults,
	...
	public static ArrayList<Ontology_Struc> makeOntologyStructures(JSONArray jsonArr,
			ArrayList<Ontology_Struc> ontologies) {
	...
	public String writeDataMatching(ArrayList<String> schemaChildren, Match_Struc matchDetails, Query_Data queryData,
			ArrayList<Ontology_Struc> ontologies) {
		String dataDetails = "";
	...
	private String writeObject(String paramName, Match_Struc matchDetails, Query_Data queryData) {
		...
		// System.out.println("Create_Query:writeObject: literalObject "+
		// literalObject);
		...
		if (literalObject != null) {
			...
			// If the literal is more than a simple quoted string then the datatype or
			// language information will be picked out of the query at this stage
