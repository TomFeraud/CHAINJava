...
/**
 * Filter the target schemas so as to return only those which are related to the
 * query schema
 * 
 * @author Diana Bental (November 2017)
 *
 */
public class Narrow_Down {
	...
	/**
	 * Given the head (or predicate) from the query, and one or more target
	 * schemas, return only the related target schemas.
	 * 
	 * @param queryHead
	 * @param targetSchemas
	...
	public static String narrowDown(String queryHead, String targetSchemas) {
		...
		// Use WordNet to build a set of words (or terms) associated with any of the
		// words in the query head
		Set<String> associatedWords = new HashSet<String>();
		try {
			IDictionary dict = JWI_Caller.openDictionary();
		...
		// For each target schema in turn - do any of the words in the head match any of
		// the associaed words?
		...
		// Return to the same format as the original, i.e. a semi-colon separated string
		// of schemas
